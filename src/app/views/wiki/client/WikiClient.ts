...
class WikiClient extends GeneralClient {
    ...
    async removeNavigationItem(id: number) {
        (await this.client!.post("/api/wiki/config/navigation/remove", { id }));
    }
    ...
    async getWikiConfig<MenuAsText extends boolean>(menuAsText: boolean): Promise<GetWikiConfigResponse<MenuAsText>> {
    ...
    async createNewVersion(page: number, version: number, content: string, navigation_id: number | null, comment: string) {
        await this.client!.post("/api/wiki/newversion", {
            page, version, content, navigation_id, comment
        });
    }
    async createNewPage(title: string, content: string, navigation_id: number | null): Promise<{ pageID: number }> {
